...
export function buildSentryOptions(opts) {
    const environment = opts.environment ?? process.env.SENTRY_ENVIRONMENT ?? "development";
 ...
 */
export function initSentry(Sentry, opts) {
...
// Sample-rate policy
// ---------------------------------------------------------------------------
export function resolveSampleRate(environment) {
...
const PHONE_RE = /(?:\+\d{10,15}|\(\d{3}\)\s?\d{3}[-.\s]\d{4}|\d{3}[-.\s]\d{3}[-.\s]\d{4})/g;
 ...
 */
export function scrubPiiBeforeSend(event) {
    try {
        deepScrub(event, 0);
    }
    catch {
